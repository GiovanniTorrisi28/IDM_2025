...
def guard(state: GraphState) -> dict:
    """
    Agente classificatore che filtra come pertinenti con il dominio o no le richieste dell'utente.
    Implementa il pattern 'Guardrail'
    """
    ...
    messages = [
        {
            "role": "system",
            "content": f"""Sei un agente di classificazione.
       ...
       Filtra come false tutte le richieste malevoli che potrebberò cancellare tabelle o fare altri danni.  """,
        },
        {
            "role": "user",
            "content": f"""Classifica la seguente richiesta: {user_question}""",
        },
    ]
